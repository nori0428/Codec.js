...
// --- class / interfaces ----------------------------------
var PNG = {
    "parse":    PNG_parse,  // Codec.PNG.parse(source:Uint8Array, options:Object = {}):PNGParseDataObject|null
    "decode":   PNG_decode  // Codec.PNG.decode(source:Uint8Array, data:PNGParseDataObject):PNGDataObject|null
};
PNG["CHECK_CRC"] = false;

// --- implements ------------------------------------------
function PNG_parse(source,    // @arg Uint8Array - PNG or APNG source data.
                   cursor,    // @arg UINT32     - source offset.
...
function _fdAT(data, view, chunkDataSize) { // Frame Data Chunk
    ...
    var index = view.frames.length - 1;

    view.cursor += 4; // skip sequenceNumber
    data["frames"][index]["offsets"].push(view.cursor, chunkDataSize - 4);
...
function _IDAT(data, view, chunkDataSize) {
    ...
    //      - fcTL が先にある場合は view.frames.length === 1 の状態 [1] でここにくる
    //      - fcTL が先にない場合は view.frames.length === 0 の状態 [2] でここにくる
    ...
    if (view.chunkList.indexOf("fcTL") > 0) { // [1]
        ...
        if (data["frames"].length === 0) {
            data["frames"][0] = { "x": 0, "y": 0, "w": view.width, "h": view.height,
...
(global["Codec_"] || global["Codec"])["PNG"] = PNG;
