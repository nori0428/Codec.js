...
var SIZE_FROM_TYPE = {};
SIZE_FROM_TYPE[TYPE_FIX_EXT1] = 1;
SIZE_FROM_TYPE[TYPE_FIX_EXT2] = 2;
SIZE_FROM_TYPE[TYPE_FIX_EXT4] = 4;
SIZE_FROM_TYPE[TYPE_FIX_EXT8] = 8;
SIZE_FROM_TYPE[TYPE_FIX_EXT16] = 16;
 ...
 */
    ...
    var view = {
            ...
            extenc:     options["extenc"] || null, // extenc(data:Any):Object - { data:Uint8Array, type:INT8  }
...
function MessagePack_decode(source,    // @arg Uint8Array - source
                            options) { // @arg Object = {} - { ascii, extdec, copy }
                                       // @options.ascii Boolean = false - String is FIX_STR and ASCII only.
                                       // @options.extdec Function = null - TYPE_EXT decode function.
...
//{@dev
    ...
    $valid($keys(options, "ascii|buffer|extenc|extdec|copy"), MessagePack_decode, "options");
    if (options) {
 ...
 */
    ...
    var view = {
            ...
            ascii:      options["ascii"]  || false,
            extdec:     options["extdec"] || null, // extdec(data:Uint8Array, type:INT8):Any
            cursor:     0,
...
function _encodeMessagePackNumber(source, view) {
    ...
    if (source !== source) {
        ...
        if (source >= -32) {                // [TYPE_FIX_INT | 0xNNNNN]
         ...
         // buffer.set([TYPE_INT64, high >> 24, high >> 16, high >>  8, high,low  >> 24, low  >> 16, low  >>  8, low], cursor);
         // cursor += 9;
        ...
        if (source < 0x80) {                // [TYPE_FIX_UINT]
         ...
         // buffer.set([TYPE_UINT64, high >> 24, high >> 16, high >>  8, high, low  >> 24, low  >> 16, low  >>  8, low], cursor);
         // cursor += 9;
...
function _encodeMessagePackExt(source, view) {
    var r = view.extenc(source); // extenc(data:Any):Object - { data:Uint8Array, type:INT8 }
    if (!r) {
        return; // ignore
    }
    var type = r["type"];
    ...
    case 1: case 2: case 4: case 8: case 16:
        view.buffer[view.cursor++] = TYPE_FIX_EXT1 + size - 1;
...
function _decodeMessagePack(source, // @arg Uint8Array
                            view) { // @arg Object - { extdec, ascii, cursor, double, float, byte }
                                    // @ret Any
                                    // @recursive
    ...
    if (type >= TYPE_NIL && type <= TYPE_INT64) {
        ...
        case TYPE_INT64:    if (source[view.cursor] & 0x80) {
                            ...
                            /* falls through */
        case TYPE_UINT64:   high = (source[view.cursor++] << 24 | source[view.cursor++] << 16 |
                                    source[view.cursor++] << 8  | source[view.cursor++]) >>> 0;
                            low  = (source[view.cursor++] << 24 | source[view.cursor++] << 16 |
                                    source[view.cursor++] << 8  | source[view.cursor++]) >>> 0;
