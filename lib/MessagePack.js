...
function MessagePack_encode(source,    // @arg Any
                            options) { // @arg Object = {} - { askey, ascii, buffer, extenc, quickly }
                                       ...
                                       // @options.quickly Boolean = false - quickly
                                       // @ret Uint8Array - return new view of buffer.
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(options, "Object|omit"), MessagePack_encode, "options");
        $valid($keys(options, "askey|ascii|buffer|extenc|extdec|copy|quickly"), MessagePack_encode, "options");
        if (options) {
            ...
            $valid($type(options.quickly,"Boolean|omit"),    MessagePack_encode, "options.quickly");
        }
    }
//}@dev
    options = options || {};
    ...
    var view = {
            askey:      options["quickly"] || options["askey"]  || false,
            ascii:      options["quickly"] || options["ascii"]  || false,
...
function MessagePack_decode(source,    // @arg Uint8Array - source
                            options) { // @arg Object = {} - { askey, ascii, extdec, copy, quickly }
...
//{@dev
    if (!global["BENCHMARK"]) {
        ...
        $valid($keys(options, "askey|ascii|buffer|extenc|extdec|copy|quickly"), MessagePack_decode, "options");
        if (options) {
