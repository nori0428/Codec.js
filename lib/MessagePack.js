...
var MessagePack = {
    ...
    "pack":   MessagePack_pack,   // Codec.MessagePack.pack(source:Any):Object - { data:Uint8Array, type:INT8 }
    "unpack": MessagePack_unpack, // Codec.MessagePack.unpack(data:Uint8Array, type:INT8):Any
    "float":  MessagePack_float,
    "unpacker": MessagePack_unpacker
};
...
function MessagePack_float(n) {
    this._n = n;
}
MessagePack_float.prototype = new Number();
MessagePack_float.prototype.valueOf = function() {
    return this._n;
};
MessagePack_float.prototype.toString = function() {
    return this._n.toString();
};

function MessagePack_unpacker() {
    var that = {
        chunk: []
    };
    that.feed = function(data) { //@arg Any
        if (typeof data === "string") {
            that.chunk = that.chunk.concat(data.split(''));
        } else {
            that.chunk = that.chunk.concat(data);
        }
    };
    that.unpack = function() { //@ret Any or undefined
        if (that.chunk.length < 1) {
            return undefined;
        }
        var source = new Uint8Array(that.chunk);
        var r = MessagePack_decode(source);
        if (r.data !== undefined) {
            that.chunk = that.chunk.slice(r.cursor);
        }
        return r.data;
    };
...
//}@dev
    ...
    return {data: _decodeMessagePack(source, view), cursor: view.cursor};
}
...
function _encodeMessagePackAny(source, view, depth) {
    ...
    if (source === null || source === undefined) {
        ...
        default:
            ...
            } else if (source instanceof MessagePack_float) {
                _encodeMessagePackNumber(source, view);
...
function _decodeMessagePack(source, // @arg Uint8Array
    ...
    if (type >= TYPE_NIL && type <= TYPE_INT64) {
        ...
        case TYPE_INT32:    if (source.length - view.cursor < 4) return undefined;
        ...
        case TYPE_INT16:    if (source.length - view.cursor < 2) return undefined;
                            size =  source[view.cursor++] << 8  | source[view.cursor++];
                            return size < 0x8000     ? size : size - 0x10000;
        case TYPE_INT8:     if (source.length - view.cursor < 1) return undefined;
                            size =  source[view.cursor++];
                            return size < 0x80       ? size : size - 0x100;
        case TYPE_INT64:    if (source.length - view.cursor < 8) return undefined;
        ...
        case TYPE_UINT32:   if (source.length - view.cursor < 4) return undefined;
                            return (source[view.cursor++] << 24 | source[view.cursor++] << 16 |
                                    source[view.cursor++] << 8  | source[view.cursor++]) >>> 0;
        case TYPE_UINT16:   if (source.length - view.cursor < 2) return undefined;
                            return  source[view.cursor++] << 8  | source[view.cursor++];
        case TYPE_UINT8:    if (source.length - view.cursor < 1) return undefined;
                            return  source[view.cursor++];
        case TYPE_FLOAT64:  if (source.length - view.cursor < 8) return undefined;
                            view.cursor += 8;
        ...
        case TYPE_FLOAT32:  if (source.length - view.cursor < 4) return undefined;
                            view.cursor += 4;
    ...
    if (type <= TYPE_FIX_STR_MAX) {
        ...
        case TYPE_STR32: case TYPE_BIN32: case TYPE_EXT32: case TYPE_MAP32:  case TYPE_ARRAY32:
            if (source.length - view.cursor < 4) return undefined;
            size = (source[view.cursor++] << 24 | source[view.cursor++] << 16 |
                    source[view.cursor++] << 8  | source[view.cursor++]) >>> 0;
            break;
        case TYPE_STR16: case TYPE_BIN16: case TYPE_EXT16: case TYPE_MAP16:  case TYPE_ARRAY16:
            if (source.length - view.cursor < 2) return undefined;
            size = source[view.cursor++] << 8 | source[view.cursor++];
            break;
        case TYPE_STR8:  case TYPE_BIN8: case TYPE_EXT8:
            if (source.length - view.cursor < 1) return undefined;
            size = source[view.cursor++];
        }
    }

    var object, key, len, i = 0, d;
    ...
    case TYPE_STR8:
    case TYPE_FIX_STR:
        if (source.length - view.cursor < size) return undefined;
        view.cursor += size;
...
// http://gyazo.com/671cc7ec8b59f5da3b7ffa11c0672ac6.png
        while (size--) {
            d = _decodeMessagePack(source, view);
            if (d === undefined) {
                return undefined;
            }
            object.push(d);
    ...
    case TYPE_FIX_MAP:
        ...
        if (view.askey) {
            ...
            while (size--) {
                ...
                if (source.length - view.cursor < len) return undefined;
